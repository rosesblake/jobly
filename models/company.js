...
class Company {
  ...
  static async findAll(q = {}) {
    ...
    if (
      Object.keys(q).some(
        (key) => !["name", "minEmployees", "maxEmployees"].includes(key)
      )
    ) {
    ...
    if (q.minEmployees) {
      whereClause += " AND num_employees >= $1";
      queryParams.push(q.minEmployees);
    }
    if (q.maxEmployees) {
      if (q.minEmployees) {
        whereClause += " AND num_employees <= $2";
      } else {
        whereClause += " AND num_employees <= $1";
      }
      queryParams.push(q.maxEmployees);
    }
    if (q.minEmployees > q.maxEmployees) {
      throw new BadRequestError("minEmployees must be less than maxEmployees");
    }
    //WHERE 1=1 so query is not required.
